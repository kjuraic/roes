...
oes_plot_compare <- function(nist, oes_lst){
  ...
  manipulate::manipulate({
    ggplot2::ggplot(data = nist) +
      ggplot2::geom_segment(ggplot2::aes(x = wavelength, xend = wavelength, y = 0, yend = rel.*fac, color= element)) +
      ggplot2::theme_bw()+ ggplot2::ylim(0,1) +
      ggplot2::xlim(w_min, w_max) +
      ggplot2::geom_line(data = oes_lst[[id]]$data, ggplot2::aes(x = lambda + w_shift, y = I/max(I)*fac_I))
  ...
  id = manipulate::slider(min = 1, max = length(oes_lst), initial = 1, step = 1)
  )
}
