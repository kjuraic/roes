...
#' @example
#'     read_nist_oes('data/Database/AR_I.txt')
read_nist_oes <- function(file_name) {
  if(file.exists(file_name)){
    pos <- c(1,10,19,30,41,52,63,74,76,90,1/01,112,123,134,145,173,185,201,211,240)
    col_names <- stri_trim_both(strsplit(x = readLines(file_name, n = 2)[2], split = "|", fixed = TRUE)[[1]])
    col_names <- stri_replace(col_names, fixed = " ", "_")
    col_names[1] <- 'Element'
    col_names <- stri_trans_tolower(col_names)
    #col_names <- stri_replace_all(str = col_names, '_', fixed=' ')
    #dat <- read.fwf(file = file, widths = diff(pos),skip = 5)
    #names(dat) <- col_names
    dat = suppressWarnings(read_fwf(file = file_name, fwf_widths(diff(pos), col_names), skip = 5,
                                    col_types = cols(wavelength = 'd', rel. = 'd', aki = 'd',  fik = 'd', s = 'd',
...
read_nist_oes_multi <- function(element_list, db_path = 'extdata/Database/'){
  ...
  df <- adply(.data = fnms, .margins = 1, .fun = read_nist_oes)
  df
}
...
nist_db_content <- function(){
  tmp = list.files("extdata/Database/")
  tmp = stri_replace(tmp, "", fixed = ".txt")
  return(tmp)
}
